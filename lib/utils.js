...
module.exports = {
    ...
    /* Sync Platform */
    /* ------------------------------------------------------------------------------------------------ */
    syncPlatform: function( platform ) {
        try {
            console.log( 'Syncing ' + platform + ' platform' );
            const child = spawnSync( 'npx' , [ 'cap' , 'sync' , platform ] , { encoding : 'utf8' } );
            console.log( child.stdout );
            if ( child.status !== 0 ) {
                throw child.stderr;
            }
            console.log( platform + ' platform synced' );
            return true;
        } catch( error ) {
            console.log( error );
            return false;
    ...
    emberServe: function() {
        try {
            const child = spawnSync( 'ember' , [ 'serve' ] , { encoding : 'utf8' } );
            return child.status === 0;
        } catch( error ) {
            console.log( error );
            return false;
    ...
    /* Init Capacitor */
    /* ------------------------------------------------------------------------------------------------ */
    initCapacitor: function( name , id ) {
        try {
